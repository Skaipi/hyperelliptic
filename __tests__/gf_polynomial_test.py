...
def test_constructors():
    ...
    assert gf.element([1, 0, 0, 1, 0, 0]) == gf.poly_one
    assert gf.element([1, 0, 0, 1, 0, 1]) == gf.poly_zero
...
def test_addition():
    ...
    assert p1 + gf.poly_one == gf.element([1, 0, 1, 0, 1, 0])
...
def test_substraction():
    ...
    p2 = gf.element([1, 0, 1, 0])
    assert p1 + p2 == p1 - p2
    assert p1 - gf.poly_one == gf.element([1, 1, 1, 1])
    assert p1 - gf.poly_zero == p1
...
def test_division():
    ...
    assert divmod(p1, p2) == (gf.element([1, 0, 0]), gf.poly_zero)
    assert divmod(p1, p3) == (gf.element([1, 0, 1, 0]), gf.poly_zero)
    assert divmod(p2, p3) == (gf.element([1, 0]), gf.poly_one)
